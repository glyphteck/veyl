...
2. Read [guidelines/map.md](guidelines/map.md) or run `bun map` for the fastest architecture and active-work orientation.
3. Read [guidelines/workflow.md](guidelines/workflow.md) for task sizing, todo, branch, worktree, handoff, and cleanup rules.
4. Read the focused files under [guidelines/](guidelines/) that match the task.
5. Check the root docs if the task touches their area:
...
- Do not use `todo/` to list completed work. When a task is done, delete its task file or trim it to only unresolved work, then document implemented behavior in the focused guide that owns it. Update README only when the human overview changes.
...
- After a feature branch is merged, delete the merged feature branch and any abandoned predecessor branches, then document the shipped behavior in the focused guide that owns it. Update AGENTS or README only when repo-wide rules or the human overview change.
...
- Track the actual diff with Git (`git status --short`, `git diff`) or `bun dirty`; use todo files for coordination and handoff context, not as a substitute for Git.
...
- [guidelines/map.md](guidelines/map.md): compact architecture map, feature owners, active-work orientation, and fast commands
