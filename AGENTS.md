...
- Do not depend on VS Code tasks for repo operations. The repo intentionally has no `.vscode/` workflow source; use the pnpm scripts documented in [guidelines/commands.md](guidelines/commands.md), including `pnpm push` and `pnpm merge`.
...
11. Keep Markdown docs current with the repo state. Planning docs should describe pending work, while durable docs should describe implemented behavior.

## Guideline Index

- [guidelines/repo.md](guidelines/repo.md): repo shape, pnpm workspace, product model, account model, data model
