...
Current implementation decision: keep root route swiping app-owned and narrow. `apps/veyl/ios/src/lib/homepager.js` uses Expo Router's `TabRouter` as the selected-tab source of truth, renders the four home routes in `react-native-pager-view`, disables tab-level back history, and ignores stale native page-selection events unless they match the live scroll position while the pager is actively dragging or settling. Home tab order, root-path gating, last-route mapping, and warming live in `apps/veyl/ios/src/lib/hometabs.js`. `MainMenu` derives its icon animation from the pager's live `position`.
...
- `apps/veyl/ios/app/_layout.js`
- `apps/veyl/ios/app/(vault)/_layout.js`
- `apps/veyl/ios/app/(vault)/(app)/_layout.js`
- `apps/veyl/ios/app/(onboarding)/_layout.js`
- `apps/veyl/ios/src/lib/stackoptions.js`
- `apps/veyl/ios/app/(vault)/(app)/(home)/_layout.js`
- `apps/veyl/ios/src/lib/homepager.js`
- `apps/veyl/ios/src/lib/hometabs.js`
- `apps/veyl/ios/app/(vault)/(app)/(home)/camera.js`
- `apps/veyl/ios/package.json`
...
## iOS SDK Patch Needs Device Retest

Status: in progress

The SDK 56 patch matrix has been applied, and both `bun x expo install --check` and `bun check:frameworks` report dependencies are up to date.
...
The dev iOS build now installs and launches on `zak 15` with bundle id `com.glyphteck.veyl.dev`. Video upload and video thumbnail generation have been verified on device after the working-tree fixes, but this update should not be treated as a fix for swipe-to-leave routes until that flow is verified separately.

Fix direction:

- Retest the route where swipe-to-leave fails.
- Run the standalone test build after the dev build behavior is known.
