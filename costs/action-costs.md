...
| Send media or long text | Text-message cost plus signed chat-media upload call and one Storage upload | ~$0.000027 before stored bytes on active-notification path |
...
| Save message forever | `setChatMessageTtl` sets the shared message doc `ttl` to `null` | ~$0.000008 |
| Save media forever | Save message forever plus one Storage temporary-hold metadata update | ~$0.000013 |
| Unsave media | `setChatMessageTtl` restores a normal TTL and clears the Storage temporary hold | ~$0.000013 |
...
| Delete one text message | Callable hard-deletes the source message doc | ~$0.000006 |
| Delete one media message | Text-message hard delete plus Storage hold-clear/delete cleanup | ~$0.000011 |
...
| Send 1 media item | ~$0.000027 before stored bytes |
| Save 10% of sent messages/media | ~$0.000021 + retained storage growth |
| Search once | ~$0.000006 |
| **Total immediate daily active user ops** | **~$0.00047/day + stored-byte run-rate** |

Equivalent operation bundle per daily active user:

- about 324 Firestore read-equivalent operations,
