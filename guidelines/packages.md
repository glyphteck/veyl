...
`bun make ios` and `bun make ios dev` are the normal phone rebuild paths for the dev client. They run a clean prebuild, build, install `dev.veyl` on the configured phone, and do not start Metro. Use `bun dev ios` for the dev-client server. Use `bun make ios reset` after app identity, signing, associated-domain, or bundle-id changes when on-device state must be cleared.
