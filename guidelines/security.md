...
Upload abuse control is enforced with server-minted reservations. Chat media and report evidence direct Storage creates must first call the matching reservation callable, which validates auth, path, content type, file size, account age, and the shared daily account-upload quota. New accounts are capped at 50 MiB/day; established accounts start at 250 MiB/day. Quota docs live in `usage_quotas` with hashed identities only, and reservation docs are short-lived TTL records read by Storage rules. Do not reopen direct `media/` or report-evidence Storage creates without a reservation check.
