...
- Push routing is a deliberate metadata exception: the `push` callable sees authenticated sender uid and requested recipient uid so it can enforce bans, blocks, rate limits, inbox delivery, and notifications. Logs must keep those ids redacted, and the callable must not receive plaintext content or chat-list preview data.
