...
- Owner chat entry bodies are encrypted, but their owner path, entry id, and `ts` query marker are plaintext metadata. Backend/admin access can count a user's chat-list entries and see their list-order timestamps, but cannot read the peer chat key, peer uid, settings, or preview from the owner entry without the user's chat private key.
- Push routing is a deliberate metadata exception: the `push` callable sees authenticated sender uid and requested recipient uid so it can enforce bans, blocks, rate limits, inbox delivery, and notifications before writing the sealed inbox ping. Logs must keep those ids redacted, and the callable must not receive plaintext content or chat-list preview data. The callable returns only generic success; ping ids, push-route counts, and delivery details stay server-private. Do not move push delivery to an inbox trigger unless the replacement keeps sender authorization before recipient-visible writes and does not add plaintext sender metadata to inbox ping docs.
...
- Wallet transaction history must display only product-visible transfer rows: peer `TRANSFER` rows, recognized funding rows, and withdrawals. Spark swap/preimage coordination transfers stay out of history and cache even when they are completed and owned by the wallet.
...
Production logs and browser/device console output are not a telemetry channel for this app. App-owned logs must run only in a verbose local dev instance. Firebase Functions debug logs require `VEYL_VERBOSE=1` plus the local emulator, web logs require `NEXT_PUBLIC_VEYL_VERBOSE=1` in a non-production build, and iOS logs/diagnostics require a verbose `__DEV__` build. Never log raw `chatId`, `linkId`, media paths, wallet public keys, chat public keys, push tokens, report paths, vault state, plaintext errors with embedded payloads, or unredacted uids.
...
Authenticated public profile search/list in small batches is an accepted product exposure for client-side user search. Do not treat usernames, avatar state, public chat keys, public wallet keys, active flags, or profile ids as secrets, but keep batch limits tight and do not add broader profile enumeration paths without a product decision.
