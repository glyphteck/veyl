...
- The chat security model is dumb server, smart client powered by cryptography. The server checks owner paths, auth, bans, and bounded shapes; clients derive pair ids, decrypt owner entries and wakes, pin per-chat actor keys, verify action signatures/authenticators, and ignore invalid rows.
- Messages are encrypted before storage.
- A pair-encryption key proves that one participant could create a ciphertext, but it does not prove which participant authored it. Message ownership, edits, and payment confirmations need actor signatures derived from user-held secrets, not from the pair key. Global message deletion can use either participant's actor signature or a shared per-chat action authenticator because either participant is allowed to delete.
- User read receipts, reactions, hidden checkpoints, edits, payment confirmations, deletes, and system rows are encrypted append-only actions in the message stream. Do not add plaintext per-user read state, reaction state, hidden state, active-chat state, edit state, payment state, delete state, or retention mode to chat docs.
- `chats/{chatId}` docs should only carry opaque document version and timestamp data. Do not reintroduce participants, plaintext sender keys, plaintext read markers, plaintext previews, or plaintext chat preferences.
- User chat rows belong in owner-only encrypted `users/{uid}/chats/{entryId}` entries. Inbox wakes belong in `users/{uid}/chatInbox/{wakeId}` and must remain sealed pointers, not duplicated message content.
...
- Never use `lastMsgTime`; owner chat entry `ts` is the chat-row recency source of truth. Parent `chats/{chatId}.ts` is not a row source.
...
- Delete is a signed/authenticated tombstone. Either participant may delete any shared chat message; a valid tombstone hides the message, removes matching owner saved records, and releases saved media holds after the client processes it.
...
Direct Firestore message writes cannot maintain a true rate counter without another write/read or a callable send path. The zero-extra-write server guard is payload-size enforcement in Firestore rules for encrypted message bodies, owner entries, saved records, and inbox wakes. If message or wake volume itself becomes the abuse bottleneck, route sends/invites through a server-owned counter path instead of trying to hide a counter in client state.
