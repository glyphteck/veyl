...
Color should come from `useTheme()` and `src/lib/colors.js`: `background`, `glassBackground`, `foreground`, `glassForeground`, `muted`, `border`, `destructive`, `inflow`, `outflow`, `bitcoin`, and `active`. `background` and `foreground` are solid fill and text colors, while `glassBackground` and `glassForeground` are material tints for `GlassView` surfaces. Use `inputGlassTint(theme)` for softer input glass defaults and `disabledGlassTint(theme)` for disabled glass buttons/icons; do not dim `GlassView`-based controls with opacity. Message bubbles should use plain `View` surfaces with the shared `bubbleStyle`/`bubbleShadow` helpers from `apps/ios/src/lib/chat/messages.js`, not `GlassView` or the input tint. Avoid hardcoded colors except for camera overlays or other isolated native surfaces.
