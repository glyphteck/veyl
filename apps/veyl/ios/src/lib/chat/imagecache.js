...
import { sleep, waitForIdle } from "@veyl/shared/utils/async";
import { CHAT_MESSAGE_FILE_CACHE_MAX_BYTES } from "@veyl/shared/config";
...
function getCacheUri(key, mimeType, options = {}) {
  ...
  const namedExt = fileExtension({ name: options?.fileName });
  const ext = (namedExt.length <= 8 ? namedExt : "") || options?.defaultExt || mimeExtension(mimeType);
...
function pruneResolved() {
  ...
  while (
    ...
    totalBytes > CHAT_MESSAGE_FILE_CACHE_MAX_BYTES
  ) {
